...
void HTMLElement::parseMappedAttribute(Attribute* attr)
{
    ...
    else if (attr->value() == "return false" || attr->value().startsWith("return false;"))
        return;
// standard events
...
void HTMLElement::setContentEditable(Attribute* attr) 
{
    ...
    if (enabled.isEmpty() || equalIgnoringCase(enabled, "true")) {
        ...
        addCSSProperty(attr, CSSPropertyWebkitTextSizeAdjust, CSSValueNone);
        ...
        attr->decl()->removeProperty(CSSPropertyWebkitTextSizeAdjust, false);
...
{
    return !disabled() && Element::willRespondToMouseMoveEvents();
}

bool HTMLElement::willRespondToMouseWheelEvents()
{
    return !disabled() && Element::willRespondToMouseWheelEvents();
}

bool HTMLElement::willRespondToMouseClickEvents()
{
    return  !disabled() && Element::willRespondToMouseClickEvents();
}
