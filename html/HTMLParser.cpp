...
PassRefPtr<Node> HTMLParser::parseTelephoneNumbers(Node *inputNode)
{
    ...
    if (outputNode->isTextNode()) {
        ...
        do {
            if (parentNode->isLink()  || parentNode->isCommentNode() || parentNode->hasTagName(scriptTag) || (parentNode->isHTMLElement() && (static_cast<HTMLElement*>(parentNode.get()))->isGenericFormElement()) || parentNode->hasTagName(styleTag) || parentNode->hasTagName(ttTag) || parentNode->hasTagName(preTag) || parentNode->hasTagName(codeTag)) {
