...
void RenderBox::computeRectForRepaint(RenderBoxModelObject* repaintContainer, IntRect& rect, bool fixed)
{
    ...
    if (o->hasOverflowClip()) {
        ...
        if (!containerBox->layer()->hasAcceleratedTouchScrolling()) {
        IntRect repaintRect(topLeft, rect.size());
    ...
    if (h.isPercent()) {
        ...
        if (containingBlock->isRenderView()) {
            ...
            if (view->hasCustomFixedPosition(this))
                return computeContentBoxLogicalHeight(h.calcValue(customContainingBlockAvailableLogicalHeight(containingBlock->style(), view)));
        }
...
IntRect RenderBox::layoutOverflowRectForPropagation(RenderStyle* parentStyle) const
{
    ...
    bool hasTransform = hasLayer() && layer()->transform();
    if (isRelPositioned() || (hasTransform && (!document()->settings() || document()->settings()->shouldTransformsAffectOverflow()))) {
