                                           ...
                                           bool& affectsAscent, bool& affectsDescent) const
{
    ...
    if (usedFonts && !usedFonts->isEmpty() && (includeFont || (box->renderer()->style(m_firstLine)->lineHeight().isNegative() && includeLeading)) && !box->renderer()->document()->settings()->alwaysUseBaselineOfPrimaryFont()) {
