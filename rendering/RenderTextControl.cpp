...
void setSelectionRange(Node* node, int start, int end)
{
    ...
    if (frame)
        previousSelection = frame->selection()->selection();
...
{
    // This entire method will be moved to another class once we merge r90885.
    // For now, we need to stop using the deprecated Position methods.
...
float RenderTextControl::getAvgCharWidth(AtomicString family)
{
    ...
    TextRun textRun = TextRun(str, false, 0, 0, TextRun::AllowTrailingExpansion, LTR, false, TextRun::NoRounding);
...
int RenderTextControl::innerLineHeight() const
{
    if (m_innerText && m_innerText->renderer() && m_innerText->renderer()->style())
        return m_innerText->renderer()->style()->computedLineHeight();
