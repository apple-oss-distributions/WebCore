...
void RenderBlock::adjustRectForColumns(IntRect& r) const
{
    ...
    bool isHorizontal = isHorizontalWritingMode();
    int beforeBorderPadding = borderBefore() + paddingBefore();
    int colHeight = colInfo->columnHeight();
    if (!colHeight)
        return;

    int startOffset = max(isHorizontal ? r.y() : r.x(), beforeBorderPadding);
    int endOffset = min<int>(isHorizontal ? r.maxY() : r.maxX(), beforeBorderPadding + colCount * colHeight);
    ...
    if (startColumn == endColumn) {
        ...
        int logicalLeftOffset = logicalLeftOffsetForContent();
        IntRect colRect = columnRectAt(colInfo, startColumn);
