...
// In TOT WebCore, RenderContainer::addLineBoxRects has a different signature from RenderObject::addLineBoxRects which
// would sometimes give unexpected results.  We can remove this fix once we sync back up to TOT WebCore.
void RenderContainer::addLineBoxRects(Vector<IntRect>& rects, unsigned start, unsigned end, bool useSelectionHeight)
{
...
void RenderContainer::addLineBoxQuads(Vector<FloatQuad>& quads, unsigned start, unsigned end, bool useSelectionHeight)
{
    if (!m_firstChild && (isInline() || isAnonymousBlock())) {
        int x, y;
        absolutePositionForContent(x, y);
        absoluteQuads(quads, x, y);
        return;
    }
