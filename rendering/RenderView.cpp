...
#if !ENABLE(HW_COMP)
    // It's not so horribly wrong, since it might be a tiled layer
    ASSERT(!needsLayout());
#endif
...
void RenderView::repaintViewRectangle(const IntRect& ur, bool immediate)
{
    ...
    if (!elt)
        ...
        IntRect vr = viewRect();
        // Don't clip using the visible rect since clipping is handled at a higher level on Purple.
        IntRect r = ur;
...
void RenderView::absoluteQuads(Vector<FloatQuad>& quads, int tx, int ty, bool)
{
    FloatRect curRect(tx, ty, m_layer->width(), m_layer->height());
...
#if ENABLE(HW_COMP)
// Compositing layer dimensions take outline size into account,
// so we have to recompute layer bounds when it changes. This
// causes ugly redraw flashes; would be nice to have a better
// way to do this.
...
#if ENABLE(HW_COMP)
void RenderView::ensureRootCompositingLayer()
{
    if (m_rootLayer)
        return;
...
{
    // fprintf(stderr, "noteUpdateContents %d %d %d %d\n", r.x(), r.y(), r.width(), r.height());
    
    if (m_layer && (r.width() > 0 && r.height() > 0)) {
        m_layer->repaintAbsoluteRectIncludingDescendants(r);
    }
}

void RenderView::noteUpdate()
{
    if (m_layer) {
        m_layer->repaintIncludingDescendants();
    }
}

void RenderView::willBeDetached()
{
    if (m_rootLayer && m_rootLayerAttached) {
        m_rootLayer->setParentLayer(0);
        m_rootLayerAttached = false;
    }
}

void RenderView::wasAttached()
{
    if (m_rootLayer) {
        m_rootLayer->setParentLayer(frameView()->frame()->contentsHostingLayer());
