...
uint32_t RenderStyle::hashForTextAutosizing() const
{
    ...
    hash ^= WTF::FloatHash<float>::hash(inherited->specified_line_height.value());
...
#if ENABLE(IOS_TEXT_AUTOSIZING)
Length RenderStyle::specifiedLineHeight() const { return inherited->specified_line_height; }
void RenderStyle::setSpecifiedLineHeight(Length v) { SET_VAR(inherited, specified_line_height, v); }
#else
