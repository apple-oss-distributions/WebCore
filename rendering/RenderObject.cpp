...
bool RenderObject::requiresLayer() const
{
    return isRoot() || isPositioned() || isRelPositioned() || isTransparent() || hasOverflowClip() || hasTransformOrPerspective();
}
...
{
    RenderBlock* container = containingBlock();
    return container ? container->availableWidth() : 0;
}

int RenderObject::containingBlockHeight() const
{
    RenderBlock* container = containingBlock();
    return container ? container->contentHeight() : 0;
}
...
void RenderObject::addLineBoxQuads(Vector<FloatQuad>&, unsigned startOffset, unsigned endOffset, bool useSelectionHeight)
{
    // overridden by subclasses
}

void RenderObject::absoluteQuads(Vector<FloatQuad>& quads, int tx, int ty, bool topLevel)
{
    // For blocks inside inlines, we go ahead and include margins so that we run right up to the
    // inline boxes above and below us (thus getting merged with them to form a single irregular
    // shape).
    if (topLevel && continuation()) {
        FloatRect curRect(tx,
                          ty - collapsedMarginTop(),
                          width(),
                          height() + collapsedMarginTop() + collapsedMarginBottom());
        
        quads.append(convertRectToPageQuad(curRect, tx, ty));
        ...
        quads.append(convertRectToPageQuad(FloatRect(tx, ty, width(), height() + borderTopExtra() + borderBottomExtra()), tx, ty));
    }
}

FloatQuad RenderObject::absoluteQuadWithTransforms()
{
    int x, y;
    absolutePosition(x, y);

    // gather rects like absoluteBoundingBoxRect (but avoid calling absolutePosition again)
    ...
    return convertRectToPageQuad(boundingRect, x, y);
}
    
void RenderObject::absolutePositionForContentWithTransforms(int& xPos, int& yPos, bool fixed)
{
    absolutePositionForContent(xPos, yPos, fixed);
    FloatPoint nodePoint, pagePoint;
    nodePoint.setX(xPos);
    nodePoint.setY(yPos);
    pagePoint = convertToPage(nodePoint);
    xPos = (int)floor(pagePoint.x());
    yPos = (int)floor(pagePoint.y());
}    
...
void RenderObject::repaint(bool immediate)
{
    ...
    bool interceptRepaint = false;

#if ENABLE(HW_COMP)
    FrameView* frameView = view->frameView();
    if (frameView->hasCompositingLayers())
    {
        RenderLayer*    parentLayer = enclosingLayer();
        // the document layer is special on iPhone
        RenderLayer* compLayer = parentLayer->enclosingCompositingLayer();
        interceptRepaint = compLayer && !compLayer->isDocumentLayer();
        if (interceptRepaint) {
            IntRect objectRect   = absoluteClippedOverflowRect();

            int x = 0;
            int y = 0;
            parentLayer->convertToLayerCoords(parentLayer->root(), x, y);
            objectRect.move(-x, -y);
            parentLayer->setNeedsRepaintInRect(objectRect);
        }
    }
#endif
...
void RenderObject::repaintRectangle(const IntRect& r, bool immediate)
{
    ...
    if (frameView->hasCompositingLayers())
    {
        ...
        interceptRepaint = compLayer && !compLayer->isDocumentLayer();
        if (interceptRepaint) {
            IntRect layerRect(absRect);
            int x = 0;
            int y = 0;
            parentLayer->convertToLayerCoords(parentLayer->root(), x, y);
            layerRect.move(-x, -y);
            parentLayer->setNeedsRepaintInRect(layerRect);
        }
    }
#endif
...
#if ENABLE(HW_COMP)
// not really hardward compositing-specific. repaintRectangle() could call this.
void RenderObject::repaintAbsoluteRectangle(const IntRect& absRect, bool immediate)
{
    // Can't use view(), since we might be unrooted.
    RenderObject* o = this;
    while (o->parent())
    ...
    if (!o->isRenderView())
        return;
    RenderView* renderView = static_cast<RenderView*>(o);

    bool interceptRepaint = false;

    // begin HW_COMP-specific
    FrameView* frameView = renderView->frameView();
    if (frameView->hasCompositingLayers())
    {
        RenderLayer*    parentLayer = enclosingLayer();
        interceptRepaint = true;
        IntRect layerRect(absRect);
    ...
    // end HW_COMP-specific

    if (!interceptRepaint)
        renderView->repaintViewRectangle(absRect, immediate);
}
#endif
...
{
    if (!isText() && style) {
        style = animationController()->updateAnimations(this, style);
    }
    if (style)
        setStyle(style);
}

bool RenderObject::startAnimation(double beginTime, int index)
{
    return layer() ? layer()->startAnimation(beginTime, index) : false;
}

bool RenderObject::startTransition(double beginTime, int property, RenderStyle* fromStyle, RenderStyle* toStyle)
{
    return layer() ? layer()->startTransition(beginTime, property, fromStyle, toStyle) : false;
}

void RenderObject::setAnimationStartTime(double t)
{
    animationController()->setAnimationStartTime(this, t);
}

void RenderObject::setTransitionStartTime(int property, double t)
{
    animationController()->setTransitionStartTime(this, property, t);
}

void RenderObject::animationFinished(const String& name, int index, bool reset)
{
    if (layer())
        layer()->animationFinished(name, index, reset);
}

void RenderObject::transitionFinished(int property)
{
    if (layer())
        layer()->transitionFinished(property);
}

void RenderObject::suspendAnimations()
{
    if (layer())
        layer()->suspendAnimations();
}

void RenderObject::resumeAnimations()
{
    if (layer())
        layer()->resumeAnimations();
}

// convert the given point, in render coordinates, to document coordinates,
// taking transforms into account.
FloatPoint RenderObject::convertToPage(const FloatPoint& inPoint)
{
    // map via layers, since those know about transforms (even when animations
    // etc are running)
    RenderLayer*  closestLayer = enclosingLayer();
    RenderObject* closestLayerObject = closestLayer ? closestLayer->renderer() : 0;

    int myX, myY;
    if (isRenderInline()) {
        IntRect boxRect = absoluteBoundingBoxRect();
        myX = boxRect.x();
        myY = boxRect.y();
    }
    else
        absolutePosition(myX, myY);

    if (closestLayerObject) {
        int layerObjX, layerObjY;
        closestLayerObject->absolutePosition(layerObjX, layerObjY);
        
        FloatPoint offsetInLayerObject((float)(myX - layerObjX) + inPoint.x(), (float)(myY - layerObjY) + inPoint.y());
        return closestLayer->convertToPage(offsetInLayerObject);
    }

    return FloatPoint((float)myX + inPoint.x(), (float)myY + inPoint.y());
}

// convert from page coordinates to the coordinates of this renderer,
// taking transforms into account
FloatPoint RenderObject::convertFromPage(const FloatPoint& inPoint)
{
    // map via layers, since those know about transforms (even when animations
    // etc are running)
    RenderLayer*  closestLayer = enclosingLayer();
    ...
    if (closestLayerObject) {
        // convert point into the coords of the layer's renderer
        FloatPoint offsetInLayerObject = closestLayer->convertFromPage(inPoint);

        // compute offset from this to closestLayer's renderer
        int layerObjX, layerObjY;
        closestLayerObject->absolutePosition(layerObjX, layerObjY);

        return FloatPoint(offsetInLayerObject.x() - (float)(myX - layerObjX), offsetInLayerObject.y() - (float)(myY - layerObjY));
    }

    return FloatPoint(inPoint.x() - (float)myX, inPoint.y() - (float)myY);
}

FloatQuad RenderObject::convertRectToPageQuad(const FloatRect& inRect, int tx, int ty)
{
    // map via layers, since those know about transforms (even when animations
    // etc are running)
    RenderLayer*  closestLayer = enclosingLayer();
    ...
    FloatQuad   theQuad(inRect);
    if (closestLayerObject) {
        int layerObjX, layerObjY;
        closestLayerObject->absolutePosition(layerObjX, layerObjY);
        theQuad.move(-layerObjX, -layerObjY);
        closestLayer->convertQuadToPage(theQuad);
    }
...
void RenderObject::setStyle(RenderStyle* style)
{
    ...
    if (hasLayer()) {
        // inform the layer that our style is going to change soon,
        // so that the layer can avoid caching stale state as a result
        // of dirtyZOrderLists and repaint calls
        layer()->setStyleChangePending(true);
    }
    ...
    if (m_style) {
        ...
        if (style) {
            ...
            if ((m_style->hasAutoZIndex() != style->hasAutoZIndex() ||
                ...
                layer()->dirtyStackingContextZOrderList();
    ...
    if (hasLayer())
        layer()->setStyleChangePending(false);
...
FloatQuad RenderObject::absoluteCaretQuad(int offset, EAffinity affinity, int* extraWidthToEndOfLine)
{
    IntRect theRect = caretRect(offset, affinity, extraWidthToEndOfLine);

    // make the rect relative to this renderer
    int absx, absy;
    absolutePositionForContent(absx, absy);
    return convertRectToPageQuad(theRect, absx, absy);
}
...
void RenderObject::destroy()
{
    ...
    animationController()->cancelAnimations(this);
...
#if ENABLE(HW_COMP)
void RenderObject::scheduleLayerUpdate()
{
    if (isRenderView()) {
        FrameView* view = static_cast<RenderView*>(this)->frameView();
        if (view)
            view->scheduleLayerUpdate();
    } else if (parent()) {
        FrameView* v = view() ? view()->frameView() : 0;
        if (v)
            v->scheduleLayerUpdate();
    }
}
#endif
