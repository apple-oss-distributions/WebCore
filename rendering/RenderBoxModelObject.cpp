...
static bool allCornersClippedOut(const RoundedIntRect& border, const IntRect& clipRect)
{
    IntRect boundingRect = border.rect();
    if (clipRect.contains(boundingRect))
        return false;

    RoundedIntRect::Radii radii = border.radii();

    IntRect topLeftRect(boundingRect.location(), radii.topLeft());
    if (clipRect.intersects(topLeftRect))
        return false;

    IntRect topRightRect(boundingRect.location(), radii.topRight());
    topRightRect.setX(boundingRect.maxX() - topRightRect.width());
    if (clipRect.intersects(topRightRect))
        return false;

    IntRect bottomLeftRect(boundingRect.location(), radii.bottomLeft());
    bottomLeftRect.setY(boundingRect.maxY() - bottomLeftRect.height());
    if (clipRect.intersects(bottomLeftRect))
        return false;

    IntRect bottomRightRect(boundingRect.location(), radii.bottomRight());
                                       ...
                                       BackgroundBleedAvoidance bleedAvoidance, bool includeLogicalLeftEdge, bool includeLogicalRightEdge)
{
    ...
    if (haveAllSolidEdges && outerBorder.isRounded() && allCornersClippedOut(outerBorder, info.rect))
        outerBorder.setRadii(RoundedIntRect::Radii());
...
void RenderBoxModelObject::paintBoxShadow(const PaintInfo& info, const IntRect& paintRect, const RenderStyle* s, ShadowStyle shadowStyle, bool includeLogicalLeftEdge, bool includeLogicalRightEdge)
{
    ...
    for (const ShadowData* shadow = s->boxShadow(); shadow; shadow = shadow->next()) {
        ...
        if (shadow->style() == Normal) {
            ...
            if (hasBorderRadius) {
                ...
                RoundedIntRect influenceRect(shadowRect, border.radii());
