...
void InlineTextBox::paintSelection(GraphicsContext* context, const FloatPoint& boxOrigin, RenderStyle* style, const Font& font)
{
    ...
    UNUSED_PARAM(style);
    UNUSED_PARAM(font);
    // Never draw selection on iOS, ever EVER!
    return;
}
...
void InlineTextBox::paintCompositionBackground(GraphicsContext* context, const FloatPoint& boxOrigin, RenderStyle* style, const Font& font, int startPos, int endPos)
{
    ...
    FloatRect effectRect = font.selectionRectForText(TextRun(textRenderer()->text()->characters() + m_start, m_len, textRenderer()->allowTabs(), textPos(), m_expansion, expansionBehavior(), direction(), m_dirOverride || style->visuallyOrdered()), localOrigin, selHeight, sPos, ePos);
    effectRect.inflate(-0.5f);
    context->setStrokeColor(stroke, style->colorSpace());
    context->strokeRect(effectRect, 1.0f);
    effectRect.inflate(-0.5f);
    context->fillRect(effectRect, c, style->colorSpace());
}
...
void InlineTextBox::paintDecoration(GraphicsContext* context, const FloatPoint& boxOrigin, int deco, const ShadowData* shadow)
{
    ...
    do {
        ...
        if (deco & UNDERLINE) {
            ...
            context->drawLineForText(FloatPoint(localOrigin.x(), localOrigin.y() + baseline), width, isPrinting);
        }
        if (deco & OVERLINE) {
