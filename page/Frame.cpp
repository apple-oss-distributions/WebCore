...
void Frame::scrollOverflowLayer(RenderLayer *layer, const IntRect &visibleRect, const IntRect &exposeRect)
{
    ...
    if (!visibleRect.intersects(exposeRect)) {
        ...
        int scrollWidth = box->scrollWidth();
        if (eleft <= 0)
        ...
        int scrollHeight = box->scrollHeight();
        if (etop <= 0)
...
int Frame::checkOverflowScroll(OverflowScrollAction action)
{
    ...
    else if (pos.x() > visibleRect.maxX()) {
    ...
    else if (pos.y() > visibleRect.maxY()) {
    ...
    if (action == PerformOverflowScroll && (deltaX || deltaY)) {
        ...
        if (visiblePos.isNotNull()) {
            ...
            if (selection()->granularity() != CharacterGranularity) {
                sel.expandUsingGranularity(selection()->granularity());
            }
            if (selection()->shouldChangeSelection(sel)) {
...
void Frame::setSelectionChangeCallbacksDisabled(bool b)
{
    m_selectionChangeCallbacksDisabled = b;
}
