...
void Page::setAudioSessionCategory(unsigned int category)
{
    static unsigned int webkitAudioSessioinCategory;
    ...
    UInt32 newSessionCategory = static_cast<UInt32>(category);
    UInt32 categoryOverride = static_cast<UInt32>(Settings::audioSessionCategoryOverride());
    if (categoryOverride && categoryOverride != newSessionCategory)
        return;
    ...
    if (!categoryOverride) {
        ...
        if (newSessionCategory == kAudioSessionCategory_AmbientSound && webkitAudioSessioinCategory == kAudioSessionCategory_MediaPlayback)
            return;

        webkitAudioSessioinCategory = newSessionCategory;
    }

    AudioSessionSetProperty(kAudioSessionProperty_AudioCategory, sizeof(newSessionCategory), &newSessionCategory);
}
