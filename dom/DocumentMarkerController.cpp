...
void DocumentMarkerController::addMarker(Range* range, DocumentMarker::MarkerType type, String description, const Vector<String>& interpretations, const RetainPtr<id>& metadata)
{
    ...
    for (TextIterator markedText(range); !markedText.atEnd(); markedText.advance()) {
        ...
        int exception = 0;
        addMarker(textPiece->startContainer(exception),
                  DocumentMarker(type, textPiece->startOffset(exception), textPiece->endOffset(exception), description, interpretations, metadata));
    }
}
...
void DocumentMarkerController::addDictationPhraseWithAlternativesMarker(Range* range, const Vector<String>& interpretations)
{
    ...
    for (TextIterator markedText(range); !markedText.atEnd(); markedText.advance()) {
        RefPtr<Range> textPiece = markedText.range();
        int exception = 0;
        DocumentMarker marker(DocumentMarker::DictationPhraseWithAlternatives, textPiece->startOffset(exception), textPiece->endOffset(exception), "", Vector<String>(numAlternatives), RetainPtr<id>());
...
void DocumentMarkerController::addDictationResultMarker(Range* range, const RetainPtr<id>& metadata)
{
    ...
    for (TextIterator markedText(range); !markedText.atEnd(); markedText.advance()) {
        ...
        int exception = 0;
        addMarker(textPiece->startContainer(exception),
                  DocumentMarker(DocumentMarker::DictationResult, textPiece->startOffset(exception), textPiece->endOffset(exception), String(), Vector<String>(), metadata));
    }
}
