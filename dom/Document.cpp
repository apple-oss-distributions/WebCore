...
#if PLATFORM(IOS)
Document::Document(Frame* frame, const KURL& url, unsigned documentClasses, bool isSynthesized)
#else
Document::Document(Frame* frame, const KURL& url, unsigned documentClasses)
    ...
    , m_deviceOrientationController(DeviceOrientationController::create(frame ? frame->page() : 0, m_deviceOrientationClient.get()))
...
void Document::suspendActiveDOMObjects(ActiveDOMObject::ReasonForSuspension why)
{
    ...
    if (WebThreadCountOfObservedContentModifiers() > 0)
        if (frame() && frame()->page() && frame()->page()->chrome().client())
...
#if PLATFORM(IOS)
    // If the document is synthesized, we optimize and avoid parsing the response header
